...
class PlayerAnalysisPage:
    ...
    def __init__(self):
        """Initialize player analysis page"""
    ...
    def render(self):
        ...
        with tab1:
            self._render_enhanced_player_filters(df, key_prefix="overview_")
    ...
    def _render_enhanced_player_filters(self, df, key_prefix: str = ""):
        ...
        with col1:
            ...
            if 'position_name' in df.columns:
                positions = st.multiselect("Position", df['position_name'].unique(), key=f"{key_prefix}position_filter")
            else:
                positions = []
        ...
        with col2:
            ...
            if 'team_name' in df.columns:
                teams = st.multiselect("Team", df['team_name'].unique(), key=f"{key_prefix}team_filter")
            else:
                teams = []
        ...
        with col4:
            ...
            min_points = st.number_input("Min Total Points", 0, 300, 0, help="Minimum total points threshold", key=f"{key_prefix}min_points")
            min_form = st.slider("Min Form", 0.0, 10.0, 0.0, 0.1, help="Minimum form rating", key=f"{key_prefix}min_form")
    ...
    def _display_filtered_results(self, filtered_df):
        ...
        with col1:
            sort_by = st.selectbox("Sort by", display_cols, index=4 if 'total_points' in display_cols else 0, key="overview_sort_by")
        with col2:
            ascending = st.checkbox("Ascending order", value=False, key="overview_sort_order")
        
        # Ensure all required columns exist
        for col in display_cols:
            if col not in filtered_df.columns:
                filtered_df[col] = 'N/A'
                logger.warning(f"Missing column {col} in player data")

        # Sort and display
        sorted_df = filtered_df[display_cols].sort_values(
            by=sort_by, 
            ascending=ascending,
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "points_per_mil": st.column_config.NumberColumn("PPM", format="%.1f"),
    ...
    def _render_player_comparison_tool(self, df):
        ...
        selected_players = st.multiselect(
            ...
            key="player_comparison_multiselect"
        )
        ...
        with ai_tab3:
            ...
            if 'selected_by_percent' in display_df.columns and 'total_points' in display_df.columns:
                ...
                if not hidden_gems.empty:
                    # Calculate points per million if not already calculated
                    if 'points_per_million' not in hidden_gems.columns:
                        hidden_gems['points_per_million'] = hidden_gems['total_points'] / hidden_gems['now_cost']
                    
                    # Safe calculation of gem score with error handling
                    max_ppm = hidden_gems['points_per_million'].max() if not hidden_gems.empty else 1
                    max_form = hidden_gems['form'].max() if 'form' in hidden_gems.columns else 10
                    
                    hidden_gems['gem_score'] = (
                        (hidden_gems['points_per_million'] / max_ppm) * 0.4 +
                        (hidden_gems['form'].fillna(0) / max_form) * 0.3 +
