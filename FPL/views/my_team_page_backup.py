...
class MyTeamPage:
    ...
    def render(self):
        ...
        try:
            ...
            st.error(f"❌ Error in My Team page: {str(e)}")
    ...
    def _render_team_page(self):
        ...
        # Debug information - make this more robust
        try:
            st.write("🔍 **Debug Info:**")
        ...
        try:
            ...
            self._render_team_overview(team_data)
            ...
            # Render each tab content in its own error boundary
            with tabs[0]:
                try:
                    self._display_current_squad(team_data)
                except Exception as e:
                    logger.error(f"Error in Current Squad tab: {str(e)}")
                    st.error("Error loading squad data")
            
            with tabs[1]:
                try:
                    self._display_performance_analysis(team_data)
                except Exception as e:
                    logger.error(f"Error in Performance Analysis tab: {str(e)}")
                    st.error("Error loading performance analysis")
            
            with tabs[2]:
                try:
                    self._display_recommendations(team_data)
                except Exception as e:
                    logger.error(f"Error in Recommendations tab: {str(e)}")
            ...
            with tabs[3]:
                try:
                    ...
                    logger.error(f"Error in Starting XI Optimizer tab: {str(e)}")
                    st.error("Error in optimization")
            
            with tabs[4]:
                try:
                    self._display_swot_analysis(team_data)
                except Exception as e:
                    logger.error(f"Error in SWOT Analysis tab: {str(e)}")
                    st.error("Error in SWOT analysis")
            
            with tabs[5]:
                try:
                    self._display_advanced_analytics(team_data)
                except Exception as e:
                    logger.error(f"Error in Advanced Analytics tab: {str(e)}")
                    st.error("Error in analytics")
            
            with tabs[6]:
                try:
                    self._display_transfer_planning(team_data)
                except Exception as e:
                    logger.error(f"Error in Transfer Planning tab: {str(e)}")
                    st.error("Error in transfer planning")
            
            with tabs[7]:
                try:
                    self._display_performance_comparison(team_data)
                except Exception as e:
                    logger.error(f"Error in Performance Comparison tab: {str(e)}")
                    st.error("Error in performance comparison")
            
            with tabs[8]:
                try:
                    self._display_fixture_analysis(team_data)
                except Exception as e:
                    logger.error(f"Error in Fixture Analysis tab: {str(e)}")
                    st.error("Error in fixture analysis")
            
            # Add team reload option at the bottom
            st.sidebar.button(
                "🔄 Load Different Team",
            ...
            logger.error(f"Error rendering My Team page: {str(e)}")
    ...
    def _render_team_overview(self, team_data: dict):
